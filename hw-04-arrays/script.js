...
const addRandomMarks = (pairsWithThemes, minMark, maxMark) => pairsWithThemes.map((pairs) => [...pairs, Math.floor(Math.random()* (maxMark - minMark + 1) + minMark)]);


const pairs = getPairs(students);
const pairsWithThemes = joinPairsWithThemes(pairs, themes);
const gradedStudents = addMarks(students, marks);
const gradedPairsWithThemes = addRandomMarks(pairsWithThemes, 1, 5);
